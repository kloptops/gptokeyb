...
void deadzone_calc(int &x, int &y, int scale, int deadzone, DZ_MODE mode) {
  Vector2D stick_input((float)(x) / (float)(scale), (float)(y) / (float)(scale));
  Vector2D stick_output;
  float dz = (float)(deadzone) / (float)(scale);
  ...
  x = (int)(stick_output.x * scale);
  y = (int)(stick_output.y * scale);
}

// Options for analog scaling are:
//   dz_axial, dz_radial, dz_scaled_radial, dz_sloped_axial, dz_sloped_scaled_axial, dz_hybrid
